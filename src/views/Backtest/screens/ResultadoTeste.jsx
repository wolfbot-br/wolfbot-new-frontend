...
class ResultadoTeste extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.result !== prevProps.result) {
      ...
      let lucro_total = parseFloat(
        this.props.result.result.result.profit
      ).toFixed(2);
      let percentual_total = parseFloat(
        this.props.result.result.result.percentage * 100
      ).toFixed(2);
      let taxas = 0;
      let data = [];
      for (let i = 0; i <= resultado.length - 1; i++) {
        data.push({
          ...
          percentual_obtido:
            (resultado[i].percentualGanho * 100).toFixed(2) + "%"
        });
  ...
  render() {
    ...
    return (
      <Row>
        ...
        <Col lg="3">
          <Widget
            header={"$ " + this.state.lucro_total}
            mainText="Lucro Total"
            icon="fa fa-dollar"
            color="success"
          />
        </Col>
        <Col lg="3">
          <Widget
            header={this.state.percentual_total + " %"}
            mainText="% Lucro Total "
            icon="fa fa-percent"
            color="warning"
          />
        ...
        <Col lg="3">
          <Widget
            header={this.state.negociacoes + ""}
            mainText="Nº Negociações"
            icon="fa fa-exchange"
            color="info"
          />
        </Col>
        <Col lg="3">
          <Widget
            header={"$ " + this.state.taxas}
            mainText="Taxas Exchange"
            icon="fa fa-frown-o"
            color="danger"
          />
        </Col>
